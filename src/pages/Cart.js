...
function Cart() {
  const {carts} = useSelector((elem)=> elem.user);  return (
...
<div className='common__section my-3 flex items-center justify-center text-white font-bold text-[35px]'>
   Your Cart Here
      ...
      <div className='flex items-center text-center flex-col space-y-4'>
        ...
        {carts.length >= 1 ? <p className='font-bold text-[23px]'>Carts Item : {carts.length}</p>
         :<button className='bg-[#fd2020] py-2 px-3 text-[17px] text-white'>
        No Item Selected</button>}
      </div>
...
<p className='text-[#212245] font-bold text-[18px]'>{title}</p>.com
