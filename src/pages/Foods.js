  ...
  return (
   ...
   All Foods Here
      ...
      <div className='md:mx-12 sm:mx-6 mx-3 lg:mx-20 xl:mx-24'>
        ...
        <input className='flex-1 px-2 dark:text-[#fff] bg-transparent dark:placeholder:text-[#fff] outline-none placeholder:text-[17px] placeholder:text-[#333] 
        font-medium ' type="text" placeholder="Search here..." onChange={(event) => {
            setSearchTerm(event.target.value);
          }} />
          <p className='dark:bg-green-500 bg-yellow-400 flex items-center justify-center cursor-pointer h-9 w-9 rounded-full'><FcSearch className='footer-i'/></p>
        </div>
        <div>
     ...
     <div className='grid sm:grid-cols-2 grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3'>

       {
            item?.filter((val) => {
              ...
              .map((val) => {
                return(
                  <div key={val.id} className='border-2 flex flex-col space-y-3 items-center
                  mx-2 max-w-[300px] md:max-w-[350px] py-4 border-gray-200 bg-gray-100 '>
                  
                  <img className='w-24 h-24 hover:scale-110 cursor-pointer translate-y-2 transition duration-500' src={val.image} alt='' />
                  <p className='text-[#212245] font-bold text-[18px]'>{val.title}</p>
                  <p className='flex items-center justify-between w-full px-3'>
                  <p className='text-[#fd2020] font-bold text-[22px]'>{val.price}</p>
